...
public class SoundManager : Singleton<SoundManager>
{
    ...
    {
        if (bgmPlayer.clip == bgmClips[(int)type])
        {
            return;
        }
    ...
    public void BgmSoundSetting(string name)
    {
        switch (name)
        {
            case "GameField":
                ...
                break;
            case "Dungeon_1":
                PlayBGM(eTYPE_BGM.Deongun, 0.8f);
                break;
            case "GolemBoss":
                PlayBGM(eTYPE_BGM.GolemBoss,0.8f);
                break;
        }
    }
    ...
    public async UniTaskVoid VictoryBgmPlay(string name)
    {
        PlayBGM(eTYPE_BGM.victory, 1,false);
        await UniTask.Delay((int)(bgmPlayer.clip.length * 1000));

        switch (name)
        {
            case "Golem_Boss":
                BgmSoundSetting(eTYPE_BGM.Deongun);
                break;
        }
    }
